...
const durabletask_js_1 = require("@dapr/durabletask-js");
...
class DaprWorkflowClient {
     ...
     */
    constructor(options = {}) {
        const grpcEndpoint = (0, index_1.generateEndpoint)(options);
        options.daprApiToken = (0, index_1.getDaprApiToken)(options);
        this._innerClient = this.buildInnerClient(grpcEndpoint, options);
    }
    buildInnerClient(grpcEndpoint, options) {
        let innerOptions = options?.grpcOptions;
        if (options.daprApiToken !== undefined && options.daprApiToken !== "") {
            innerOptions = {
                ...innerOptions,
                interceptors: [(0, index_1.generateApiTokenClientInterceptors)(options), ...(innerOptions?.interceptors ?? [])],
            };
        }
        return new durabletask_js_1.TaskHubGrpcClient(grpcEndpoint.endpoint, innerOptions, grpcEndpoint.tls);
    }
    /**
     ...
     */
    async scheduleNewWorkflow(workflow, input, instanceId, startAt) {
        if (typeof workflow === "string") {
            return await this._innerClient.scheduleNewOrchestration(workflow, input, instanceId, startAt);
        }
        return await this._innerClient.scheduleNewOrchestration((0, internal_1.getFunctionName)(workflow), input, instanceId, startAt);
    }
    /**
     ...
     */
    async terminateWorkflow(workflowInstanceId, output) {
     ...
     */
    async getWorkflowState(workflowInstanceId, getInputsAndOutputs) {
        const state = await this._innerClient.getOrchestrationState(workflowInstanceId, getInputsAndOutputs);
        if (state !== undefined) {
            return new WorkflowState_1.WorkflowState(state);
        }
    }
    /**
     ...
     */
    async waitForWorkflowStart(workflowInstanceId, fetchPayloads = true, timeoutInSeconds = 60) {
        const state = await this._innerClient.waitForOrchestrationStart(workflowInstanceId, fetchPayloads, timeoutInSeconds);
        if (state !== undefined) {
            return new WorkflowState_1.WorkflowState(state);
        }
    }
     ...
     *                                               or undefined if no such instance is found.
     */
    async waitForWorkflowCompletion(workflowInstanceId, fetchPayloads = true, timeoutInSeconds = 60) {
        const state = await this._innerClient.waitForOrchestrationCompletion(workflowInstanceId, fetchPayloads, timeoutInSeconds);
        if (state != undefined) {
            return new WorkflowState_1.WorkflowState(state);
        }
    }
     ...
     */
    async raiseEvent(workflowInstanceId, eventName, eventPayload) {
     ...
     */
    async purgeWorkflow(workflowInstanceId) {
     ...
     */
    async suspendWorkflow(workflowInstanceId) {
     ...
     */
    async resumeWorkflow(workflowInstanceId) {
...
exports.default = DaprWorkflowClient;
